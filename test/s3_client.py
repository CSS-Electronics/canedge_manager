...
class S3Client(Minio):
    ...
    def remove_bucket_r(self, bucket_name):
        ...
        if self.bucket_exists(bucket_name=bucket_name):
            ...
            objects = self.list_objects_v2(bucket_name=bucket_name, recursive=True)

            # Remove objects
            for obj in objects:
                self.remove_object(bucket_name=obj.bucket_name, object_name=obj.object_name)
    ...
    def list_objects_as_list(self, bucket_name, prefix=""):
        return list(self.list_objects_v2(bucket_name=bucket_name, prefix=prefix, recursive=True))

    # Put object string
    def put_object_string(self, bucket_name, object_name, string):
        data = io.BytesIO(string.encode())
        nob = data.getbuffer().nbytes
        self.put_object(bucket_name=bucket_name, object_name=object_name, data=data, length=nob)
